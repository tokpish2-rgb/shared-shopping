...
function renderDistribute() {
  const people = participantsArray();
  const items = commonItems();
  return `<section class='distribute-screen'><div class='section-head'><div><h2>Распределить покупки</h2><div class='meta'>Быстро перетащите — 1 единица. Удержите и перетащите — всё количество</div></div></div><div class='common-pool'><h3>Общий список</h3><div class='pool-items' data-drop-common='true'>${items.length ? items.map(renderPoolItem).join('') : `<span class='chip done'>Все разобрано</span>`}</div></div><div class='people-board'>${people.map(renderPersonDropCard).join('')}</div>${renderInviteCard()}<button class='button distribute-all' data-action='open-item'>${icon('plus')}Добавить покупку</button></section>`;
}

function renderPoolItem(item) {
  return `<button class='pool-item' draggable='true' data-item-id='${item.id}' data-drag-source='common' data-action='take-item' data-id='${item.id}'><span>${itemEmoji(item.name)}</span><strong>${escapeHtml(item.name)}</strong><em>${formatQty(remainingQty(item), item.unit)}</em></button>`;
}

function renderPersonDropCard(person) {
  const items = assignedItems(person.id);
  return `<div class='person-drop-card' data-drop-person='${person.id}'><div class='person-head'><div class='mini-avatar'>${renderFace(person.name)}</div><span class='person-name'>${escapeHtml(person.name)}</span></div><div class='mini-stack'>${items.length ? items.map((item) => `<div class='mini-item' draggable='true' data-item-id='${item.id}' data-drag-source='person' data-drag-person='${person.id}'><span>${itemEmoji(item.name)}</span><strong>${escapeHtml(item.name)}</strong><em>${formatQty(item.assignments[person.id], item.unit)}</em></div>`).join('') : ''}<div class='drop-hint'>Перетащите сюда</div></div></div>`;
}

function renderInviteCard() {
  const url = shareUrl();
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(url)}`;
  return `<section class='invite-card'><div><h3>Пригласить участников</h3><p>Покажите QR-код или скопируйте ссылку. Удерживайте QR-код, чтобы скопировать ссылку.</p><button class='button secondary' data-action='copy-share' type='button'>${icon('share')}Скопировать ссылку</button></div><button class='qr-button' data-action='copy-share' data-long-copy='true' type='button' aria-label='QR-код приглашения'><img alt='QR-код приглашения' src='${qrUrl}' width='116' height='116' /><span data-copy-status>Удержать</span></button></section>`;
}
...
function bindEvents() {
  ...
  appRoot.querySelectorAll(".pool-item[draggable='true'], .mini-item[draggable='true']").forEach((node) => {
    node.addEventListener('pointerdown', () => {
      ...
      node._longPressTimer = setTimeout(() => node.classList.add('drag-all-ready'), longPressMs);
    });
  ...
  node.addEventListener('pointerdown', () => {
    ...
    timer = setTimeout(async () => {
      ...
      setTimeout(() => {
        ...
        if (status) status.textContent = 'Удержать';
      }, 1400);
    }, longPressMs);
  });
...
function dragQty(node) {
  ...
  return Date.now() - startedAt >= longPressMs ? available : Math.min(1, available);
}
...
async function handleClick(event) {
  ...
  if (action === 'delete-item') await removeRemote(`items/${target.dataset.id}`);
  render();
}
...
async function createList(data) {
  ...
  state = normalizeState({ id: listId, title: data.title.trim(), adminUid: firebaseUid || `local_${uid('admin')}`, createdAt: Date.now(), participants: { [personId]: { id: personId, name: data.name.trim(), createdAt: Date.now() } }, items: {} });
...
async function joinList(name) {
  const personId = uid('person');
  const person = { id: personId, name: name.trim(), createdAt: Date.now() };
...
async function addParticipant(name) {
  const personId = uid('person');
  modal = null;
  await updateRemote(`participants/${personId}`, { id: personId, name: name.trim(), createdAt: Date.now() });
}

async function saveItem(data) {
  const id = data.id || uid('item');
  const existing = state.items[id] || {};
  const item = { ...existing, id, name: data.name.trim(), qty: Number(String(data.qty).replace(',', '.')) || 1, unit: normalizeUnit(data.unit), assignments: existing.assignments || {}, done: Boolean(existing.done), createdAt: existing.createdAt || Date.now() };
  modal = null;
